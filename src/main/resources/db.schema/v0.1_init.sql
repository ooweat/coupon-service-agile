...
CREATE FUNCTION `nextval`(SEQ_NAME VARCHAR(30)) RETURNS INT(1) UNSIGNED
    MODIFIES SQL DATA DETERMINISTIC
BEGIN
    DECLARE RET INT(1) UNSIGNED;
UPDATE SEQUENCES SET CURRVAL = IF(CURRVAL <> 9, CURRVAL + 1, 0) WHERE NAME = SEQ_NAME;
